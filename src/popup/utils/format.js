...
export function formatTickEntry(e) {
  ...
  const cy =
    e.cyclesLeft != null ? `cy=${String(e.cyclesLeft).padStart(3)}` : "cy=  ?";
  const rm =
    e.actRemaining != null
      ? `rm=${String(e.actRemaining).padStart(2)}`
      : "rm= ?";
  const ln =
    e.actLength != null ? `ln=${String(e.actLength).padStart(2)}` : "ln= ?";
  const oh = `oh=${e.overheadTicks ?? "?"}`;
  const lb =
    e.lootBagItems != null
      ? `lb=${e.lootBagItems}/${e.bankTriggerItemCount ?? "?"}`
      : "lb=?";
  const gen =
    e.itemsGenerated != null
      ? `gen=${String(e.itemsGenerated).padStart(3)}`
      : "gen=  ?";
  const bt =
    e.bankTrips != null ? `bt=${String(e.bankTrips).padStart(2)}` : "bt= ?";
  const sm =
    e.cycleSamples != null
      ? `sm=${String(e.cycleSamples).padStart(2)}`
      : "sm= ?";
  const pg =
    e.cycleProgressMs != null
      ? `pg=${(e.cycleProgressMs / 1000).toFixed(0)}s`
      : "pg=?";
  const cal = e.calibrated ? "cal=Y" : "cal=N";
  const rd = e.rawDuration != null ? `rd=${e.rawDuration}` : "rd=?";
  const dd =
    e.defDurMs != null ? `dd=${(e.defDurMs / 1000).toFixed(0)}s` : "dd=?";
  const cd =
    e.cycleDurMs != null ? `cd=${(e.cycleDurMs / 1000).toFixed(0)}s` : "cd=?";
  const od =
    e.observedCycleDurMs != null
      ? `od=${(e.observedCycleDurMs / 1000).toFixed(0)}s`
      : "od=?";
  const goal =
